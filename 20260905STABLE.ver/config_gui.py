...
("evening_start_hour","퇴근 시작",0,24,2,0),("evening_end_hour","퇴근 주요시간 종료",0,24,2,0),("evening_taxi_fraction","퇴근 택시 비율",0,1,3,0),("late_evening_start_hour","늦은 퇴근 시작",0,24,2,0),("late_evening_end_hour","늦은 퇴근 종료",0,24,2,0),("evening_residential_fraction","퇴근 목적지 주거 비율",0,1,3,0),("restaurant_stay_sec","음식점 체류시간(초)",0,86400,0,1),
],
...
class ConfigGUI:
 ...
 def _run(self,script,args=(),blocking=False):
  ...
  try:
   ...
   flags=subprocess.CREATE_NEW_CONSOLE if os.name=="nt" else 0; subprocess.Popen([sys.executable,path,*args],cwd=ROOT,env=self._env(),creationflags=flags);return True
