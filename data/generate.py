...
def generate_data(meta, start, days, seed=42):
    ...
    for date in pd.date_range(pd.Timestamp(start).normalize(), periods=days, freq='D'):
        ...
        for bucket in pd.date_range(date, periods=288, freq='5min'):
            ...
            for cell in cells:
                ...
                weather.append((bucket, cell, temp, rain, traffic, event, holiday))
    ...
    external = pd.DataFrame(weather, columns=['time_bucket', 'h3_index', 'temperature', 'precipitation', 'traffic_index', 'event_flag', 'is_holiday'])
