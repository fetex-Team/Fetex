...
[통합 이력] 예측 피처 생성을 Module 2 파이프라인(fetex.preprocessing)과 동일한
...
import pandas as pd
import traci
from fetex.core.config import CFG, ROOT
from fetex.preprocessing.time_series_prep import TimeSeriesPreprocessor
...
def _external_to_weather(external):
    ...
    if 'time_bucket' not in external or 'temperature' not in external:
        return None
    w = (external.groupby('time_bucket', as_index=False)[['temperature', 'precipitation']]
