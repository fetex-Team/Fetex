...
from fetex.geospatial.region_autocomplete import RegionSearchEntry

ROOT=str(PROJECT_ROOT); PRESET_DIR=os.path.join(ROOT,"presets"); os.makedirs(PRESET_DIR,exist_ok=True)
...
class ConfigGUI:
 ...
 def _run(self,module,args=(),blocking=False):
  command=[sys.executable,"-m",module,*args]
  try:
   if blocking:return subprocess.run(command,cwd=ROOT,env=self._env(),check=True)
   if os.name=="nt":
   ...
   subprocess.Popen(command,cwd=ROOT,env=self._env());return True
  except Exception as e:messagebox.showerror("실행 오류",str(e));return False
 def run_main(self):
  self.save_config_only();
  if self._run("fetex.simulation.build_env",blocking=True):self._run("fetex.apps.main")
 ...
 def run_training(self):
    ...
    try:
        ...
        self._run("fetex.forecasting.train", blocking=True)
        ...
        self._run(
            "fetex.forecasting.train_category_demand",
            blocking=True
        )
 ...
 def open_ab_compare_dialog(self):
  ...
  def run_compare():
   ...
   win.destroy()
   self.save_config_only()
   self._run("fetex.analysis.multi_factor_compare",[json.dumps(combos,ensure_ascii=False)])
