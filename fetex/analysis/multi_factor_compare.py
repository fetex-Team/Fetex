...
def launch_workers(combos: list) -> list:
    ...
    for idx, combo in enumerate(combos):
        ...
        if os.name == "nt":
            ...
            inner_cmd = subprocess.list2cmdline([sys.executable, "-m", "fetex.analysis.parallel_dispatch_worker", config_dir_name, label, result_path])
            ...
            subprocess.Popen(
                [sys.executable, "-m", "fetex.analysis.parallel_dispatch_worker", config_dir_name, label, result_path],
                cwd=ROOT, env=env
            )
    return result_paths
...
def compare(result_paths: list):
    from fetex.runtime.measure_wait_time import print_result
