...
from fetex.runtime.measure_wait_time import print_result, set_dispatch_algorithm_in_config

MODULE1 = os.path.join(ROOT, "fetex", "simulation")
...
def build_shared_map(algo_a: str):
    ...
    subprocess.run([sys.executable, "-m", "fetex.simulation.build_env", "--config-dir", "sumo_config_A"],
                    check=True, cwd=ROOT)
...
def launch_workers(algo_a: str, algo_b: str):
    ...
    if os.name == "nt":
        subprocess.Popen(["cmd", "/k", sys.executable, "-m", "fetex.analysis.parallel_dispatch_worker", "sumo_config_A", algo_a, "_result_A.json"],
                          cwd=ROOT, env=env, creationflags=subprocess.CREATE_NEW_CONSOLE)
        subprocess.Popen(["cmd", "/k", sys.executable, "-m", "fetex.analysis.parallel_dispatch_worker", "sumo_config_B", algo_b, "_result_B.json"],
                          cwd=ROOT, env=env, creationflags=subprocess.CREATE_NEW_CONSOLE)
    else:
        subprocess.Popen([sys.executable, "-m", "fetex.analysis.parallel_dispatch_worker", "sumo_config_A", algo_a, "_result_A.json"], cwd=ROOT, env=env)
        subprocess.Popen([sys.executable, "-m", "fetex.analysis.parallel_dispatch_worker", "sumo_config_B", algo_b, "_result_B.json"], cwd=ROOT, env=env)
