...
BOOLS=[("use_real_map","실제 OSM 지도 사용"),("dispatch_use_euclidean","Module 4 테스트 거리: 유클리드 사용"),("suppress_sumo_warnings","SUMO 경고 로그 숨기기 (속도 향상)"),("run_sim_before_training","학습만 실행 시 시뮬레이션 먼저 돌려서 로그 생성 (끄면 기존 로그로 바로 학습)")]
...
class ConfigGUI:
 ...
 def run_training(self):
        ...
        # 2. [자동 변환 로직] 택시 배차 알고리즘이 'greedy' 등 외부 제어 불가능 모드인 경우 'hungarian'으로 자동 전환
        # (SUMO traci 모드가 활성화되어 강화학습/한가리안 배차가 가능해집니다)
        if c.get("taxi_dispatch_algorithm") in ["greedy", "routeExtension"]:
            c["taxi_dispatch_algorithm"] = "hungarian"
            self.choice_vars["taxi_dispatch_algorithm"].set("hungarian")  # GUI 화면 표시도 업데이트
            print("[안내] 강화학습/외부 제어를 위해 배차 알고리즘을 'hungarian'으로 자동 변경했습니다.")

        # 3. 변경된 설정을 config.json에 저장
        with open(CONFIG_PATH, "w", encoding="utf-8") as f:
            json.dump(c, f, ensure_ascii=False, indent=2)

        try:
            # 0/4 sumo 환경(build_env.py) 재생성 (traci 모드 반영)
            ...
            # 0.5/4 train.py가 읽을 data/sim_logs/demand_log_*.csv를 채워줄 시뮬레이션 실행.
            #       이 단계 없이는 train.py가 예전/빈 로그만 보고 0행짜리 피처 테이블로
            #       train_test_split에서 바로 ValueError가 남.
            #       여러 번 돌려서 로그를 누적시킴(1회 분량이 lag/rolling/타겟 윈도우보다
            #       짧으면 dropna로 다 날아가기 때문).
            #       "공통/선택" 탭의 체크박스로 생략 가능 (기존 로그가 이미 충분할 때 시간 절약).
